...
for day in timesList:
    ...
    directory = '/Users/Elise/penn_holloway/WHIPS/GOME2/2007/'
    prefix = 'no2track' 
    today = glob.glob(os.path.join(directory, prefix+day.strftime('%Y%m%d')+'*'))
    yesterday = glob.glob(os.path.join(directory, prefix+(day-dayDelta).strftime('%Y%m%d')+'*'))
    try:
    ...
    call.extend(['--filetype', 'GOME2_NO2_KNMI_HDF_v2_3'])
    ...
    inFields = ['vcdtrop']
                #'kernel','amf',amftrop','psurf','a_lev','b_lev']
    outFields = ['tropVCD']
                #'avkern','AMF','tropAMF','TM4pSurf','TM4presLevA','TM4presLevB']
    ...
    outDir = '/Users/Elise/penn_holloway/WHIPS-Output/GOME2/'
    call.extend(['--outDirectory', outDir])
#    call.extend(['--outFileName', 'gome2_pixMin3150_pixMax5800_0_25x0_25_CONUS_'+day.strftime('%m-%d-%Y')+'_cloudfrac_0.nc'])
    call.extend(['--outFileName', 'gome2_pixMin3150_pixMax15500_0_25x0_25_CONUS_'+day.strftime('%m-%d-%Y')+'.nc'])
