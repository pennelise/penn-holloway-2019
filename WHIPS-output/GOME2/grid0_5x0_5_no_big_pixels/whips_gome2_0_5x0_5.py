...
for day in timesList:
    ...
    inFields = ['vcdtrop']
              # ['kernel','vcdtrop','amf',
              #  'amftrop','psurf','a_lev','b_lev']
    outFields = ['tropVCD']
                ['avkern','tropVCD','AMF',
                 'tropAMF','TM4pSurf','TM4presLevA','TM4presLevB']
    ...
    outDir = '/Users/Elise/penn_holloway/WHIPS-output/GOME2/grid0_5x0_5/'
    call.extend(['--outDirectory', outDir])
#    call.extend(['--outFileName', 'gome2_pixMin3150_pixMax5800_0_5x0_5_CONUS_'+day.strftime('%m-%d-%Y')+'.nc'])
    call.extend(['--outFileName', 'gome2_pixMin3150_pixMax5800_0_5x0_5_CONUS_'+day.strftime('%m-%d-%Y')+'.nc'])

    if dayone:
        call.extend(['--includeGrid', os.path.join(outDir,'CONUS_0_5x0_5_gridfile.nc')])
        dayone = False
